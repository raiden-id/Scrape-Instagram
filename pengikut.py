...
def pengikut(name,c):
	...
	uid = myuid(name,c)
	try:
		url = f"https://i.instagram.com/api/v1/friendships/{uid}/followers/?count=200&search_surface=follow_list_page"
		...
		for b in range(int(next)):
			req = ses.get(f'https://i.instagram.com/api/v1/friendships/{uid}/followers/?count=200&max_id={next}&search_surface=follow_list_page',cookies=c,headers={"user-agent":'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Instagram 213.1.0.22.117 (iPhone13,2; iOS 15_0_2; en_US; en-US; scale=3.00; 1170x2532; 332048479)'})
			x = json.loads(req.text)
			try:
				for i in x["users"]:
					id.append(i["username"]+"-"+i["full_name"])
					#print(i["username"]) 
					print(f'\r-> {len(id)}',end="\r")
